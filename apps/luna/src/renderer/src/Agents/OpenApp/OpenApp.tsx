...
interface OpenAppAgentProps {
  app: string
  result?: { success: boolean; output: string }
}

export const OpenAppAgent: React.FC<OpenAppAgentProps> = ({ app, result }) => {
  if (result) {
      ...
      <div className="flex flex-col gap-1.5 my-2 px-3 py-2 rounded-lg bg-neutral-900/60 border border-border/40 text-xs">
        <div className="flex items-center gap-2 text-muted-foreground/70">
          ...
          <span>
            ...
            <span className="text-indigo-300 font-medium">{app}</span>
          </span>
        </div>
        {result.output && (
  ...
  return (
    <div className="flex items-center gap-2 my-2 px-3 py-2 rounded-lg bg-neutral-900/60 border border-border/40 text-xs text-muted-foreground/70">
      ...
      <span>
        Opening app <span className="text-indigo-300 font-medium">{app}</span>...
      </span>
    </div>
  )
}
